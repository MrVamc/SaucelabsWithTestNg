...
public class finalCheckOut extends newBaseclass{
	...
	public void verifyCheckOutwitValidData(String firstName, String lastName, String zipCode) throws InterruptedException {
		...
		try {
			extentReports.test.info("Clicking on finish order");
			check.getFinishButton().click();
			Assert.assertEquals(check.getThankYouNote().getText(), "Thank you for your order!", "Order placed");
			extentReports.test.pass("Order placed");
		} catch (AssertionError e) {
			Assert.fail();
			extentReports.test.fail("Order not placed: "+ e);
		}
	}
