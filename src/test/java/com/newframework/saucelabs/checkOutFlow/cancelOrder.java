...
public class cancelOrder extends newBaseclass {
	...
	public void verifyCheckOutwitValidData(String firstName, String lastName, String zipCode) throws InterruptedException {
		...
		try {
			extentReports.test.info("Clicking on cancel order");
			check.getCancelOrder().click();;
			Assert.assertTrue(driver.getCurrentUrl().contains("inventory"),"Order not cancelled!!");
			extentReports.test.pass("Order cancelled");
		} catch (AssertionError e) {
			Assert.fail();
			extentReports.test.fail("Order not cancelled: "+ e);
		}
	}
