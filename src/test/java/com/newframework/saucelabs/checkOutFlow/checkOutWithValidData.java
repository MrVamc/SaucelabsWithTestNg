...
public class checkOutWithValidData extends newBaseclass{
	...
	public void verifyCheckOutwitValidData(String firstName, String lastName, String zipCode) throws InterruptedException {
		...
		try {
			extentReports.test.info("Checking out with valid data");
			Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-two"), "Check out overview not displayed!!");
			extentReports.test.pass("Check out overview displayed");
		}
		catch (AssertionError e) {
			Assert.fail();
			extentReports.test.fail("Check out overview not displayed!! "+ e);
		}
	}
